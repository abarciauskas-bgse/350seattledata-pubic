...
create or replace function election_date_2019() returns timestamp as $$
    DECLARE  
        election_date_2019 timestamp;
    BEGIN  
        return (SELECT TO_DATE('20191105','YYYYMMDD'));
    END;
$$ LANGUAGE plpgsql;
...
create or replace function count_total_eligible() returns int as $$
    ...
    BEGIN  
        return (select count(*) from voters_kc_2019);
    END;
$$ LANGUAGE plpgsql;
...
-- Select from voter table where birthday is greater than 35 years ago as of the general election in 2019.

create or replace function count_under35() returns int as $$
    DECLARE  
        under35 integer;
    BEGIN  
        return (select count(*) from voters_kc_2019 where Birthdate > (election_date_2019() - interval '35 years'));
    END;
$$ LANGUAGE plpgsql;
...
-- select from voter table where birthday is less than 50 years ago as of the general election in 2019.
create or replace function count_over50() returns int as $$
    DECLARE  
        over50 integer;
    BEGIN  
        return (select count(*) from voters_kc_2019 where Birthdate < (election_date_2019() - interval '50 years'));
    END;
$$ LANGUAGE plpgsql;
...
create or replace function count_voted() returns int as $$
    DECLARE  
        count_voted integer;
    BEGIN  
        return (select count(*) from voters_kc_2019 where VOTED = 1);
    END;
$$ LANGUAGE plpgsql;
...
create or replace function count_under35_voted() returns int as $$
    ...
    BEGIN  
        return (select count(*) from voters_kc_2019 where Birthdate > (election_date_2019() - interval '35 years') and VOTED = 1);
    END;
$$ LANGUAGE plpgsql;
...
create or replace function count_over50_voted() returns int as $$
    DECLARE  
        over50 integer;
    BEGIN  
        return (select count(*) from voters_kc_2019 where Birthdate < (election_date_2019() - interval '50 years') and VOTED = 1);
    END;
$$ LANGUAGE plpgsql;
